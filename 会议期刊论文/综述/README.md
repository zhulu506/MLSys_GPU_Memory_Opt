...
## 9) 2025_FITEE_C刊/SCI三区_Training large-scale language models with limited GPU memory: a survey

> 非常 nice 的一篇综述，对笔者有很高的参考价值，而且应该是首篇按照模型参数、模型状态和模型激活对相关技术和工作进行梳理的综述。

- 本文（第 2 章）首先**分析了训练大规模模型时导致内存消耗巨大的主要因素**。
- 随后，我们**回顾了在有限 GPU 内存下训练大规模模型的各种方法**。通过对大规模模型训练过程的分析，我们识别出模型参数（第 3 章）、模型状态（第 4 章）和模型激活（第 5 章）是 GPU 内存消耗的主要来源，并**讨论了针对这些方面的训练优化方法和技术**。
- 最后（第 6 章），我们**探讨了在有限内存条件下训练大规模模型的未来发展方向**。

摘抄：

<div style="text-align:center">
    <img src="./pictures/Training large-scale language models with limited GPU memory-图1.png" style="width:100%; height:auto;">
</div>

图 1 展示了自 2016 年以来各种深度神经网络模型规模与 GPU 内存容量的增长趋势。红色倒三角表示 DNN 模型规模，绿色点表示 NVIDIA GPU 内存容量，蓝色点表示国产 GPU 内存容量。

DNN 模型规模呈现出约**每两年增长 240 倍**的趋势，而 GPU 内存容量仅从 2016 年发布的 NVIDIA P100 GPU 的 12 GB 增长到 2023 年 NVIDIA H200 GPU 的 141 GB。尽管 2019 年之前单张 GPU 仍勉强能够支持主流模型训练，但 2020 年后迅速出现了转折点，GPT-3 的参数规模达到了惊人的 1750 亿，超过当时最先进的 NVIDIA A100 GPU 内存容量的 10 倍以上。

因此，大规模模型训练采用分布式存储/并行加速模式已成为必然。然而，这也带来了 **GPU 设备间通信的严重问题**。在过去 20 年里，GPU 的峰值计算性能提升了 90000 倍，而内存/硬件互连带宽仅提升了 30 倍。在这样的条件下，分布式并行的可扩展性受到极大限制。

<div style="text-align:center">
    <img src="./pictures/Training large-scale language models with limited GPU memory-表2.png" style="width:80%; height:auto;">
</div>

内存分配主要来源于**模型参数**（亦称模型权重）的巨大存储需求。对于包含 N 个参数的模型，当以浮点数 32（FP32）格式传输至 GPU 时，GPU 内存总共消耗 4N 以存储整个模型。

另一个重要的内存消耗来源于训练过程中的**模型状态**。在随机梯度下降（SGD，Stochastic Gradient Descent）优化算法中，需要保留反向传播过程中使用的**梯度**，这会额外占用 4N 的 GPU 内存。此外，对于 Adam 优化器，内存分配还需额外存储梯度方差，需要再增加 4N 的 GPU 内存。此外，梯度的动量和方差各自还需分配两个独立的 4N 内存。因此，Adam 优化器在训练过程中总共需要 16N 的 GPU 内存。对于 AdamW 也存在类似的内存分配模式。

除了参数和模型状态外，训练**激活**（activations）也消耗了 GPU 内存的很大一部分。假设输入批大小为 b，序列长度为 s，对于具有 l 层的 Transformer 模型，若其隐藏层大小为 h，词汇表大小为 V，则其参数数量约为 $l(12h^2 + 13h) + Vh$。当 h 充分大时，参数数量约为 $12lh^2$。对于这些激活，$M_a$ 计算公式为 $(34bsh + 5bs^2a)l$，其中 a 为模型的头数。在 GPT-4 中，$M_a$ 以“-”表示，因为其具体设置未公开。在表 2 中，我们计算了一些典型的大规模模型的参数量及其参数内存消耗 $M_p$ 和模型状态内存消耗 $M_s$，假设 $b = 16$ 且 $s = 128$，并使用 Adam 优化器。在 Adam 优化器和混合精度训练的条件下，$M_s$ 约为 $M_p$ 的 3 倍。我们注意到，GPT-3 175B 产生的激活约为 366 GB。然而，GPT-3 采用了 2048 的长序列和 3.2M 的大批大小进行训练，因此其激活的内存消耗远超过 366 GB。**随着批大小和序列长度的增加，模型激活的内存消耗也将按比例增长**。

理想情况下，$M_s$ 占据主要开销，其次是 $M_p$，然后是 $M_a$。然而，在仅能使用多 GPU 并行的非理想场景下，$M_s$ 可以通过优化器并行方法有效摊销。接下来是 $M_p$，它可以通过张量并行高效地划分到多个 GPU 以分配模型中的张量运算符参数。**$M_a$ 则是最难优化的部分**。

总的来说，模型参数、模型状态和模型激活是 GPU 内存消耗的三大关键因素。减少模型参数、模型状态和模型激活的内存消耗有多种方法。例如，
- 研究人员（第 3 章）通常采用多 GPU 训练和混合精度训练来降低模型参数的内存消耗。此外，他们设计了一些特定的模型来减少 $M_p$。
- 至于减少 $M_s$，研究人员（第 4 章）通常关注优化器，并减少梯度的内存消耗。
- 最后但同样重要的是，$M_a$ 通过重计算（rematerialization）和交换（swapping）来减少（第 5 章）。

**第 3 章 减少模型参数的内存占用**

随着**参数数量的增长**，高效训练大规模模型成为一个尚未解决的关键挑战。模型参数会导致**严重的内存冗余**，尤其是在数据并行（Data Parallel）中，因为整个模型参数会被复制到所有计算节点上。为了解决模型参数的内存冗余问题，可以采用分配（assignment）和减少（reduction）的方法。具体来说，
- 分配指的是**对模型参数进行切分，并将其分配到不同的 GPU 上，从而缓解单个 GPU 的内存压力**。
- 而减少则意味着**在保持模型性能的同时减少参数数量**。  

**第 3.1 节 多 GPU 训练**

针对单个 GPU 上**模型参数的内存使用**问题，**分布式和并行训练**尝试**将模型参数分配到不同的 GPU 上**，并已被证明在缓解参数占用方面至关重要。

在介绍用于**减少模型参数内存冗余**的分布式和并行训练方法之前，我们首先介绍数据并行。由于每个计算节点都存储完整的模型副本，**数据并行会导致大量的内存冗余**。为了减少训练整个数据集时的内存冗余，**数据并行将训练样本划分为多个子集，并在每个计算节点上分别训练这些子集**。该方法通过在多个计算节点之间分配计算负载，减少了模型参数存储带来的内存负担。每个计算节点独立执行由随机梯度算法驱动的训练过程。……**PyTorch 分布式数据并行**（PyTorch Distributed Data Parallel，PyTorch DDP）以多进程方式启动多个计算节点，每个计算节点存储完整的模型副本。所有计算节点之间使用 AllReduce 操作同步梯度。由于数据并行会在每个计算节点上复制整个模型，因此它会导致**模型带来的内存冗余**，尤其是**对于大规模语言模型来说，大量的参数会占据 GPU 内存的主要部分**。例如，BERT-Large 具有 3.4 亿个参数，仅存储这些参数就需要大约 5.07 GB 的 GPU 内存，并且这还不包括训练过程中激活值消耗的内存。

为了**减少单个 GPU 上的模型参数数量**，可以使用诸如模型并行（Model Parallel，MP）和流水线并行（Pipeline Parallel）等并行算法，**将模型参数分配到不同的 GPU**，从而降低单个 GPU 的模型参数内存消耗。这些方法已被广泛应用于**缓解模型参数的内存占用**问题。

**3.1.1 模型并行**

模型并行（Model Parallel，MP）**将模型划分为多个子模型，并将每个子模型分配到不同的 GPU 上**，例如 **DistBelief**。与数据并行（Data Parallel）相比，模型并行提供了一种**缓解单个 GPU 内存冗余**的替代方法。模型参数被分布到不同的计算节点，每个计算节点仅负责发送和接收激活（activations），而不是梯度（gradients），并专注于更新其分配的参数。

**张量模型并行（Tensor Model Parallel）**是模型并行的一种特定形式，包括列并行（Column Parallel）和行并行（Row Parallel）技术。**Megatron-LM** 的研究中展示了张量模型并行的典型应用，其中 Transformer 层内的多层感知机（MLP）和自注意力（self-attention）模块分别采用列并行和行并行策略进行划分。为了确保收敛，矩阵内部的数据同步通过 AllReduce 机制完成。  

**专家并行（Expert Parallel）**是在专家混合（Mixture of Experts, MoE）模型中发展而来的。专家并行专门用于 **MoE 模型**，其中计算核心被分配给特定的专家（experts）。本质上，专家并行可以被视为模型并行的一种变体，但它也**可以与数据并行和模型并行结合使用**。

设计和实现模型并行算法存在诸多挑战。与数据并行相比，模型并行在**可扩展性**方面存在一定的局限性，并且实现难度更大。此外，在模型扩展能力、灵活性和计算效率之间实现合适的平衡仍然是一项复杂的任务。因此，模型并行算法往往是**针对特定架构或任务设计**的，限制了其泛化能力。

**3.1.2 流水线并行** 

GPipe 由 Google 提出，主要关注**流水线并行（Pipeline Parallel）**。最初，GPipe 将模型划分为 $N$ 个部分，并将每个部分分配给特定的加速器。基于这种划分方式，GPipe **进一步将训练批次 $B$ 分割为 $M$ 个微批次（micro-batches）**，这些微批次在 $N$ 个加速器之间以流水线方式执行。在反向传播过程中，每个微批次的梯度计算使用与前向传播相同的模型参数。最终，所有 $M$ 个微批次的梯度会累积并用于更新当前小批量（minibatch）在所有加速器上的模型参数。在 PyTorch 上，GPipe 的实现为 TorchGPipe。然而，GPipe 存在严重的**流水线空泡（bubble）**问题。

PipeDream 的架构针对 GPipe 的空泡问题提出了改进，采用了一种**前向计算和反向计算交替进行（1F1B，one forward one backward）**的策略。该策略确保每个阶段的反向计算在前向计算完成后立即开始。通过减少空泡开销，PipeDream 优化了 GPU 内存的利用率。1F1B 策略已广泛应用于近期的流水线并行研究。……

**3.1.3 序列并行**

对于基于 Transformer 的语言模型，其输入通常是一个序列。然而，当输入序列较长时，自注意力（self-attention）机制的时间和空间复杂度呈二次增长，导致内存开销增加。在某些特定场景下，例如医学图像处理，输入序列长度可能过大，超出 GPU 内存的训练能力。**序列并行（Sequence Parallel）**通过**将输入序列划分为多个子块（chunks），并将每个子块分配给一个 GPU**，使得**每个 GPU 仅存储完整序列的一部分子序列**。

……

**3.1.4 混合并行**  

**混合并行（Mixed Parallel）**是数据并行（Data Parallel）、模型并行（Model Parallel, MP）和流水线并行（Pipeline Parallel）技术的统称。它引入了一种新的范式，即**三维并行（3D Parallel）**，其中 x 轴代表流水线并行，y 轴代表模型并行，z 轴代表数据并行。

（代表的有 HetPipe、DAPPLE）

除了混合并行之外，**自动并行（Automatic Parallelism）**也是一个重要的研究方向，旨在**针对大规模模型和设备网格（device mesh）自动寻找最优的并行算法**。……

此外，如何**平衡通信与计算的重叠**，以加速大规模模型的训练，仍然是一个挑战。

**第 3.2 节 混合精度训练**

**混合精度训练（Mixed-precision training）**，如 NVIDIA APEX 引入的方法，旨在**使用半精度浮点数训练深度神经网络**（DNN）。通过采用低精度方法，可以显著降低训练过程中的内存和计算需求。……尽管混合精度训练通常需要维护一份**额外的权重副本**，从而增加了内存需求，但由于激活值采用低精度表示，其对整体内存使用的影响相对较小。  

**量化训练（Quantized training）**旨在通过在推理或训练过程中执行量化来降低计算成本。作为一个有益的副作用，量化还**可以减少训练过程中的内存占用**。在训练过程中，所有层的激活值都必须存储在内存中以计算梯度。**激活压缩训练（Activation compressed training）**技术通过量化**将激活值压缩为较低数值精度**，从而节省内存。此外，**权重压缩（Weight compression）**技术和**梯度压缩（Gradient compression）**技术分别对权重和梯度进行压缩，以减少存储和通信开销。

**第 4 章 降低模型状态的内存占用**  

通常，模型状态包括**优化器状态、梯度和参数**。在本文中，模型状态仅包括优化器状态和梯度，因为我们主要介绍减少模型状态内存消耗的方法。因此，本节主要介绍**如何减少模型状态的内存冗余**。  

**第 4.1 节 ZeRO**  

训练大规模模型的主要挑战之一是由于**模型状态数量庞大**而导致的高内存需求。将此类模型加载到 GPU 内存中极具挑战性。例如，在采用混合精度训练并使用 Adam 优化器的情况下，训练具有 1750 亿参数的 GPT-3 需要大约 2.1 TB 的 GPU 内存用于存储模型状态。  

研究表明，训练过程中主要有两部分消耗内存：模型状态（model state）和残差状态（residual state）。

**模型状态**包括优化器状态、梯度和参数。以 Adam 优化器为例，除了存储模型参数外，还需要存储**每个梯度的动量和方差**，在混合精度训练中会额外占用 $12\times$ 内存。为缓解这一内存限制，ZeRO 提出了一个关键思想：**针对数据并行的优化器状态、梯度和参数分区**。在数据并行度为 $N$ 的情况下，优化器状态、梯度和参数被划分为 $N$ 组，每个 GPU 或进程仅负责存储和更新其中的 $1/N$，从而**减少数据并行导致的内存冗余**。  

**残差状态**包括激活值（activation），这在模型并行中较为常见，还包括**临时缓冲区**和**内存碎片**。
- 对于训练时的激活值，ZeRO 采用了**分区（partitioning）**策略。在模型并行中，冗余主要来自于重复存储的激活值，因此 ZeRO 通过在每一层的激活计算中进行分区并在特定时间点进行存储来消除冗余。在模型的前向传播过程中，输入激活值被分布到系统中的所有 GPU 或模型并行（MP）进程中。
- ZeRO 为训练过程中所需的临时缓冲区分配了足够大的**固定大小缓冲区**，以减少频繁的内存访问。
- 此外，ZeRO 通过在反向传播过程中**为激活值和临时梯度分配连续的内存块**，并在产生后及时管理它们，从而缓解内存碎片问题。  

ZeRO 由三个阶段组成，即 ZeRO-1、ZeRO-2 和 ZeRO-3。其中，ZeRO-1 对**优化器状态**进行分区，ZeRO-2 对**梯度**进行分区，ZeRO-3 对**模型参数**进行分区，以减少内存冗余。在通信开销方面，ZeRO-1 和 ZeRO-2 的通信量与普通数据并行相同，而 ZeRO-3 的通信开销增加了 $1.5\times$，因为共享参数需要进行聚合。  

ZeRO 受到了广泛关注，并已应用于多个项目。例如，**微软的 DeepSpeed 是一个分布式训练框架**，使研究人员能够训练任意规模的大型模型。DeepSpeed 已成功用于多个项目。此外，研究人员提出了 **Colossal-AI**，并在其框架中集成了 ZeRO 优化器。PyTorch 团队也实现了 **PyTorch 完全分片数据并行（Fully Sharded Data Parallel, FSDP）**，作为 ZeRO 优化器的另一种实现方案。

**第 5 章 减少模型激活的内存占用**

模型激活在**反向传播过程中计算梯度**方面同样起着至关重要的作用。然而，如果这些激活始终保留在 GPU 上，它们会消耗大量 GPU 内存，从而对高效的内存使用构成巨大挑战。随着批量大小或序列长度的增加，模型激活的数量也会增加。为了解决这一问题，已经提出了多种方法来减少训练过程中激活的内存消耗。其中，
- 细粒度（fine-grained）的内存优化技术是指针对**张量**的操作，
- 而粗粒度（coarse-grained）的内存优化技术则涉及**模型层**的操作。

**第 5.1 节 重计算**  

**重计算（Rematerialization）**方法涉及在深度神经网络（DNN）训练的前向传播过程中**驱逐（evicting）**层或张量，并在反向传播时重新计算它们，这种方法也被称为**检查点（checkpointing）**。  

**第 5.2 节 交换** 

**交换（Swapping）**方法涉及在 GPU 与其他物理设备（如 CPU 或固态硬盘 SSD）之间传输数据，以减少 GPU 内存使用量。交换（swapping）也被称为**卸载（offloading）**。  

**ZeRO-Offload** 构建于 ZeRO 之上，旨在通过将所有 **FP32 模型状态**和 **FP16 梯度**从 GPU 卸载到 CPU 内存来训练大规模模型。参数更新在 CPU 上执行，而 FP16 参数仍保留在 GPU 上，且前向和反向传播均在 GPU 上进行。ZeRO-Offload 采用 CPU-Adam 优化器来加速 CPU 计算，并已集成到 DeepSpeed 框架中。 

**ZeRO-Infinity** 旨在利用 CPU、GPU 和 **NVMe** 来减少大规模模型训练的内存占用。该方法引入了“无限卸载引擎”（infinity offload engine），该引擎会将**所有模型状态**卸载到 CPU 或 NVMe 内存。……ZeRO-Infinity 还包括类似 ZeRO-Offload 的**激活卸载**。……

交换方法受限于通信链路的带宽，如 PCIe。由于这些硬件限制，相较于重计算，交换方法会产生更高的开销。在这些方法中，**实现计算与通信的最大重叠**仍然是一个挑战。

**第 5.3 节 重计算和交换的结合**

结合重计算与交换技术的方法已被研究用于优化深度神经网络（DNNs）训练中的 GPU 内存使用。  

……

尽管这些策略已被提出，但在并行训练中的高效应用仍然缺乏。例如，如何高效且动态地结合三维并行（3D Parallel）和内存优化来训练采用**四维并行（4D Parallel）**的大规模模型，仍然是研究者需要解决的问题。

**第 6 章 讨论**

**4 结合内存优化方法**

目前的内存优化方法通常只关注模型参数、模型状态或模型激活中的某一部分。我们希望在训练大规模模型时能**将这些内存优化方法结合使用**。然而，目前仍缺乏相关研究来综合利用这些方法。实际上，大规模模型的训练已经采用了**超过三维并行（3D Parallel）的方法**。……然而，目前仍缺乏对这些方法的**细粒度组合研究**。……此外，近年来的**自动并行方法并未考虑内存优化**。当涉及内存优化时，对已分析的模型进行运行时优化将变得更加复杂。因此，我们期望在自动并行方法中能够考虑内存优化，**使大规模模型能够在有限的内存下自动化训练**。

**5 降低通信开销**

在分布式并行训练过程中，由于 GPU 之间的通信，性能会出现下降，尤其是在 GPU 数量增加时。这种通信包括**数据并行和张量并行中的全归约（AllReduce）通信**，以及**流水线并行中的点对点通信（point-to-point communication）**。目前，**减少通信开销**或**实现通信与计算的重叠**已成为研究热点。
